...
float3x3 CalcMatrixFromRotationScale(float4 rot, float3 scale) //Igual a mi covarianza 3d
{
...
{ //screenParams -> width, height, 0, 0
    //MatrixV -> MatrixMV -> localToWorld * view
    float4x4 viewMatrix = matrixV;
    float3 viewPos = mul(viewMatrix, float4(worldPos, 1)).xyz; //cam
    ...
    float limX = 1.3 * tanFovX; //Hecho con Hfov_focal
    float limY = 1.3 * tanFovY;
    viewPos.x = clamp(viewPos.x / viewPos.z, -limX, limX) * viewPos.z;
    viewPos.y = clamp(viewPos.y / viewPos.z, -limY, limY) * viewPos.z;

    float focal = screenParams.x * matrixP._m00 / 2; //hfov.z (Posible diferencia en el calculo)
    ...
    float3x3 T = mul(J, W);
    float3x3 V = float3x3( //Diferencia de computo

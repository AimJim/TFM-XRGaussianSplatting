Shader "Unlit/GaussianShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            StructuredBuffer<int> gaussians_order;
            ...
            v2f vert (appdata v) //Vertex shader
            {
                ...
                float3 center = get_vec3(start+POS_IDX);// float3(data[start+POS_IDX],data[start+POS_IDX+1],data[start+POS_IDX+2]);
                float3 colorVal = get_vec3(start+SH_IDX); //float3(data[start+SH_IDX],data[start+SH_IDX+1],data[start+SH_IDX+2]);
                float4 rotations = float4(data[start+ROT_IDX],data[start+ROT_IDX+1],data[start+ROT_IDX+2], data[start+ROT_IDX+3]);
                float3 scale = get_vec3(start+SCALE_IDX);//float3(data[start+SCALE_IDX],data[start+SCALE_IDX+1],data[start+SCALE_IDX+2]);
                
 
                float3x3 cov3d = computeCov3D(rotations, scale);
                
                float4 cam = mul(view, float4(center, 1.0)); 
                
                
                float4 pos2d = mul(projection, cam);
                ...
                float3x3 T = mul(J,((float3x3)view));
                ...
                o.conic = float3(cov2d[1][1], -cov2d[0][1], cov2d[0][0]) * det_inv;

          
                float2 quadwh_scr = float2(sqrt(cov2d[0][0]), sqrt(cov2d[1][1])) *3.0; 
